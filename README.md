# 🚀 Rocket Booster – A Precision Landing Challenge

**Rocket Booster** is a physics-based 3D space control game built in Unity, where precision, timing, and control are everything.

Take command of a rocket ship and navigate through hazardous environments using thrust and rotation mechanics. Avoid collisions, stabilize your movement, and land safely on the designated platform to complete each level.

---

## 🧩 Features

✅ **Physics-Based Rocket Control**  
Realistic thrust and rotation mechanics for immersive gameplay.

✅ **Precision Landing Gameplay**  
Carefully control your rocket to land safely on platforms.

✅ **Challenging Environments**  
Navigate through obstacles, terrain, and tight spaces.

✅ **Particle Effects System** 🔥  
- Real-time rocket booster flames  
- Visual feedback during thrust

✅ **Sound Effects System** 🔊  
- Thrust sound synced with movement  
- Collision and success feedback sounds  

✅ **Level-Based Progression**  
Each level introduces new challenges and layouts.

✅ **Collision System**  
Crash into obstacles and restart — rewarding careful play.

---

## 🎮 Controls

- **Thrust** → `Spacebar`  
- **Rotate Left** → `Left Arrow`  
- **Rotate Right** → `Right Arrow`  

---

## 🛠️ Developer Debug Controls (Hidden)

- **Disable Collision** → `C`  
- **Next Level Skip** → `N`  

> ⚠️ These are developer/testing shortcuts and not intended for normal gameplay.

---

## 📱 Platform

- 🎮 **Engine:** Unity 3D  
- 🖥️ **Platform:** PC (Unity Editor)  
- 📲 **Extendable to Mobile**

---

## 📸 Screenshots

### 🚀 Level Gameplay

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%201.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%202.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%203.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%204.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%205.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%206.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%207.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%208.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%209.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%2010.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%2011.png" width="800"/>
</p>

---

<p align="center">
  <img src="https://github.com/tushar2627/Rocket-Booster-/blob/main/RB%2012.png" width="800"/>
</p>

---

## 📥 Download & Play

- [Download Rocket Booster (ZIP)](https://github.com/tushar2627/Rocket-Booster-/releases/tag/v1.0)

### 🖥️ How to Run

1. Download the ZIP file  
2. Extract the folder  
3. Open the folder  
4. Run **RocketBooster.exe**

> ⚠️ Keep all files in the same folder for the game to work properly.

---

## 🔮 Future Improvements

- Fuel system with UI indicators  
- More advanced level design  
- Additional sound and visual polish  
- Scoring or time-based challenges  

---

## ❤️ Credits

- Developed by: [Tushar Saxena](https://github.com/tushar2627)  
- Game Engine: Unity  

---

## ⭐ Support

If you like this project, consider giving it a ⭐ on GitHub!

---

## 📩 Contact

📧 Email: saxenatushar301@gmail.com  
🐙 GitHub: https://github.com/tushar2627  

---

**“Control is everything. One mistake… and it’s over.”** 🚀
